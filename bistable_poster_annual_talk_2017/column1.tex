\begin{Figure}
    ...
    \TEXT{Memory and plasticity involve brain mechanisms from molecular scale to
        enormous networks. We have developed MOOSE (Multiscale Object Oriented
        Simulation Environment) to model plasticity and brain function across
        scales}.
    ...
    \begin{tikzpicture}
        ...
        \node[] (ksolve) at ([xshift=0.3\textwidth]stochastic.east) {
            \includegraphics[height=\figwidth]{./images/ksolve_dsolve.png}
        };
        \node[text width=\captionwidth] at ([yshift=-1cm]ksolve.south) {
            \TEXT{ Reaction Diffusion}};
        \node[] (hsolve) at ([xshift=0.3\textwidth]ksolve.east) {
            \includegraphics[height=\figwidth]{./images/hsolve.png}
        };
        \node[text width=\captionwidth] at ([yshift=-1cm]hsolve.south) {
            \TEXT{ Electrical: Hines}};
    \end{tikzpicture}
    \TEXT{Specialized solvers to handle computations at different scales}

    \vspace{1cm}
    \HEADING{Python Interface, GUI to create models}
    ...
    { 
        ...
        \begin{tikzpicture}[ ]
            \node [ ] (moogli)  {
                \includegraphics[height=22cm,width=0.78\textwidth]{./_images/moogli.png}
            };
        \end{tikzpicture}
    }
