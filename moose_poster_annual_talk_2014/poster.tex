...
\newcommand{\HEADING}[1]
{
    ...
    {\fontsize{1.5cm}{2em}\selectfont \textcolor{red}{#1}}
    \vspace{2cm}
}
...
\begin{Figure}
    ...
    \TEXT{Memory and plasticity involve brain mechanisms from molecular scale
    ...
    \end{tikzpicture} 
     \begin{tikzpicture}
         ...
         \foreach \j in {-3,...,3}
         {
             \node[fill=yellow!30,thin,inner sep=0pt, minimum size=3mm,circle] (n\i\j) at (\i, \j) {};
         }
         \node[below=4cm] {\CAPTION{A neuron embed in network}};
     \end{tikzpicture} 
     \begin{tikzpicture}
        \node[] (image) {
    ...
    \vspace{1cm}
    \TEXT{We have developed \textcolor{red}{MOOSE}, the Multiscale Object
    ...
    \begin{Figure}
        ...
        \SECTION{The MOOSE architecture}
        ...
        \begin{tikzpicture}
            ...
            \node[below=3.5cm] {\CAPTION{Stochastic(GSSA): GSolve}};
        \end{tikzpicture} \hfill
        \begin{tikzpicture}
            \node[] (image) {
                \includegraphics[width=0.15\textwidth]{./images/ksolve_dsolve.png}
            };
            \node[below=3.5cm] {\CAPTION{ODE + Diffusion: KSolve + DSolve}};
        \end{tikzpicture} \hfill
        \begin{tikzpicture}
            \node[] (image) {
                \includegraphics[width=0.15\textwidth]{./images/hsolve.png}
            };
            \node[below=3.5cm] {\CAPTION{Electrical: HSolve}};
        \end{tikzpicture}
...
\begin{Figure}
    ...
    \SECTION{Simulation: all-together}

    \begin{Figure}
        \begin{tikzpicture}
            \node [] (image) at (0,7) {
                \includegraphics[scale=0.4,angle=-30]{./images/single_cell.png}
            };
            \foreach \i in {-3,...,3}
            \foreach \j in {-3,...,3}
            {
                \node[circle,fill=yellow!30,inner sep=0pt,minimum size=3mm] (n\i\j) at (\i, 8+\j) {};
            }
            \node[] (image) {
                \includegraphics[width=0.4\textwidth]{./images/Gallery_Moose_Multiscale.png}
            };
    ...
    \end{Figure}
        ...
        \SECTION{Some projects using MOOSE}
