...
\begin{Figure}
    ...
    %% Solvers here
    \SECTION{Modular Solvers Available in \MOOSELOGO}

    \edef\figwidth{0.20\textwidth}

    \begin{tikzpicture}
        \node[] (image) {
            \includegraphics[width=\figwidth]{./images/stochastic_solver.png}
        }; 
        \node[text width=1.5*\figwidth] at (image.north) {\CAPTION{Chemical
    ...
    \begin{tikzpicture}
        ...
        \node[text width=1.5*\figwidth] at (image.north) {\CAPTION{Reaction
                Diffusion Solver}};
    \end{tikzpicture} \hfill
    \begin{tikzpicture}
        \node[] (image) {
        ...
        \node[text width=\figwidth] at (image.north) {\CAPTION{Electrical:
                Hines Solve}};
    \end{tikzpicture}
